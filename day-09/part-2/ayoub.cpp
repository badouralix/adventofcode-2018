...
Linked* newNode(uint64_t val, Linked* prev = NULL, Linked* next = NULL) {
...
uint64_t run(string s) {
    replace(s.begin(), s.end(), ' ', '-');
    uint64_t P = 9, N = 25;
    sscanf(s.c_str(), "%llu-players;-last-marble-is-worth-%llu-points", &P, &N);

    uint64_t high_score = 0, p = 0, scores[P];
    ...
    for (uint64_t marble = 1; marble <= N*100; marble++) {
        if (marble % 23 == 0) {
            scores[p] += marble;
