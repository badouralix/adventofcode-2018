...
class Unit:
    ...
    def __repr__(self):
        return "Unit {id} ({type}): ({x},{y}) {hp} HP {ap} AP".format(
            id=self.id, type=self.type, hp=self.hp, x=self.x, y=self.y, ap=self.ap
        )
...
class Game:
    ...
    def run(self, verbose=False):
        ...
        if verbose:
            ...
            print(self.units)
        return ELF if len(self.elves) else GOBELIN
...
class ThomasSubmission(SubmissionPy):
    def run(self, s):
        min_ap = 4
        while True:
            game = Game.parse(s, elves_ap=min_ap)
            nb_elves = len(game.elves)
            winner = game.run()
            # print(min_ap, winner, game.nb_rounds)
            # print(game)
            # print(sorted(game.units.values(), key=lambda u: (u.x, u.y)))
            # print("************************")
            if winner == ELF and len(game.elves) == nb_elves:
                return game.output()
            min_ap += 1
