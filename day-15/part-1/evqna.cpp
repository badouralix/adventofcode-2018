...
struct Entity {
    ...
    Entity(Object _type, int _x, int _y) : type(_type), x(_x), y(_y) {}
};
...
void init_entities(BoardState& state) {
    ...
    for (size_t y = 0; y < board.size(); y++) {
        for (size_t x = 0; x < board[y].size(); x++) {
            ...
            if (obj == Object::ELF || obj == Object::GOBLIN) {
                state.entities.push_back(Entity(obj, x, y));
