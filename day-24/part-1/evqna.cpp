...
struct UnitGroup {
    ...
    void attack(UnitGroup& target) {
        ...
        // cout << "Damage: " << damage << ", kills: " << units_killed << "\n";
    }
...
void resolve_attacks(Units& units, const vector<int>& targets) {
...
int winner_unit_count(Units& units) {
    int count = 0;
...
int run(const string& in) {
    Units units = parse_input(in);

    while (!army_eliminated(units)) {
        // cout << "\n********************\n";
        // for (size_t i = 0; i < units.size(); i++) {
        //     cout << "Group " << i << " contains " << units[i].n_units << " units.\n";
        // }
        // cout << "Target selection phase:\n";
        auto targets = select_targets(units);
        // cout << "Attack phase:\n";
        resolve_attacks(units, targets);
    }

    return winner_unit_count(units);
}
